...
const LeftBoard = () => {
  ...
  // Static position & scale values (replacing Leva)
  const posX = -9.6;
  const posY = 8.4;
  const posZ = -18.1;
  const scaleX = 0.8;
                ...
                >
                  ...
                  <div className="absolute bottom-1 left-1 right-1">
                    <div className="text-white bg-black/70 backdrop-blur-sm px-2 py-1 rounded text-xs text-center truncate">
                      {participant.name}
                      {participant.isLocal && <span className="ml-1 text-green-400">●</span>}
                      {pinnedParticipant && pinnedParticipant.id === participant.id && (
                  ...
                  <div className="absolute top-1 right-1">
                    <div className={`w-2 h-2 rounded-full ${participant.stream ? 'bg-green-400' : 'bg-red-400'}`}></div>
                  </div>
